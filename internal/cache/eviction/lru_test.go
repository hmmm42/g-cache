...
func TestMain(m *testing.M) {
	...
	// or initialize some global variables.
...
func TestCacheUseLRU_Basic(t *testing.T) {
	t.Run("creation", func(t *testing.T) {
		lru := NewCacheUseLRU[cache.ByteView](100, nil, nil)
		if lru == nil {
	...
	t.Run("basic operations", func(t *testing.T) {
		lru := NewCacheUseLRU[string](1024, nil, nil)

		// Test Put and Get
		lru.Put("key1", "value1")
		if v, _, ok := lru.Get("key1"); !ok || v != "value1" {
		...
		lru.Put("key1", "value2")
		if v, _, ok := lru.Get("key1"); !ok || v != "value2" {
...
func TestCacheUseLRU_EvictionOrder(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			lru := NewCacheUseLRU[string](tt.maxBytes, nil, nil)
...
func TestCacheUseLRU_MemoryManagement(t *testing.T) {
	...
	lru := NewCacheUseLRU[string](256, nil, onEvicted)
	defer lru.Stop()
...
func TestCacheUseLRU_ZeroSize(t *testing.T) {
	lru := NewCacheUseLRU[string](0, nil, nil) // Zero size means unlimited

	// Should be able to add any number of entries
	for i := 0; i < 100; i++ {
		key := "k" + string(rune(i+'0'))
...
func findKeysInSameSegment(lru *CacheUseLRU[string], n int) []string {
	seen := make(map[*segment[string]][]string)
	i := 0
	for {
